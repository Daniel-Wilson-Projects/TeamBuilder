...
<html lang="en" dir="ltr">
  ...
  background-color: #f2f2f2;
    ...
    function fetchActions(){
        ...
        var roleSelected = document.getElementById('rolesList').value;
        console.log("role"+personSelected);
        if(applicationSelected.length != 0){
          console.log('length is not == 0');


        xmlhttp1.open("POST", "fetchRoleActions", true);
        xmlhttp1.setRequestHeader("Content-type",
            "application/x-www-form-urlencoded");
        xmlhttp1.send('application='+applicationSelected+'&roleSelected='+roleSelected);
    ...
    function roleSelected(){
      var test = document.getElementById('rolesList').value;
      //if(personSelected == test){}
    ...
    //updated to look at updateRoleJsonArray
...
*/
    ...
    //  console.log(`${'${layer1}'}.${'${layer2}'}.${'${layer3}'}`);
      ...
      document.getElementById(path).checked = true;
        ...
        xmlhttp2.open("POST", "updateRoleJsonArray", true);
        xmlhttp2.setRequestHeader("Content-type",
            "application/x-www-form-urlencoded");
        xmlhttp2.send('finalJsonUpdateString='+finalForm+'&roleSelected='+roleSelected);
    ...
    //old, unused right now. keeping for reference
...
//I think all should follow this format - it limits the size of the json string sent to mergepatch to only the data key and it's children permissions and not the entire data key arrary like the others - That was done not fully understanding mergepatch limits and interactions.
        function updateJsonParentandChildrenArray(path){
          ...
          console.log('finalForm: '+finalForm);
            ...
            xmlhttp3.open("POST", "updateRoleJsonArray", true);
            xmlhttp3.setRequestHeader("Content-type",
                "application/x-www-form-urlencoded");
            xmlhttp3.send('finalJsonUpdateString='+finalForm+'&roleSelected='+roleSelected);
  ...
  <body>
    ...
    <h1>Role Permissions</h1>
     ...
     <h2>Select a role to view/grant/remove permissions</h2>
      <select id="rolesList" onchange="roleSelected()">
        <option value=""></option>
        <g:each var="role" in="${rolesList}">
         <option value="${role.ID}">${role.ORGANIZATION}: ${role.ROLE_NAME}</option>
        </g:each>
      </select>
